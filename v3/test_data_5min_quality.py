"""
Тест качества данных в таблице data_5min.
"""
...
def test_data_5min_quality():
    """Простой тест качества данных в таблице data_5min."""
    print("Тест качества данных в таблице data_5min...")
    ...
    try:
        ...
        print("\n1. Проверка структуры таблицы:")
        cursor.execute("PRAGMA table_info(data_5min)")
        ...
        print("   Колонки таблицы data_5min:")
        current_columns = {}
        for col in columns:
            col_name = col[1]
            col_type = col[2]
        ...
        print("\n2. Проверка общего количества записей:")
        cursor.execute("SELECT COUNT(*) FROM data_5min")
        total_records = cursor.fetchone()[0]
        print(f"   Всего записей: {total_records}")

        # 3. Проверка уникальности ID
        print("\n3. Проверка уникальности ID:")
        cursor.execute("SELECT COUNT(DISTINCT id) FROM data_5min")
        ...
        print("\n4. Проверка покрытия символов:")
        cursor.execute("SELECT symbol, COUNT(*) FROM data_5min GROUP BY symbol ORDER BY COUNT(*) DESC")
        ...
        print("\n5. Проверка временных данных:")
        cursor.execute("SELECT MIN(datetime), MAX(datetime) FROM data_5min")
        time_range = cursor.fetchone()
        print(f"   Период данных: {time_range[0]} - {time_range[1]}")
        
        # Проверяем формат времени
        cursor.execute("SELECT datetime FROM data_5min LIMIT 5")
        ...
        cursor.execute("SELECT COUNT(*) FROM data_5min WHERE high < low")
        high_low_error = cursor.fetchone()[0]
        print(f"   Ошибок где high < low: {high_low_error}")
        
        # Проверяем, что high >= open и high >= close
        cursor.execute("SELECT COUNT(*) FROM data_5min WHERE high < open OR high < close")
        ...
        cursor.execute("SELECT COUNT(*) FROM data_5min WHERE low > open OR low > close")
        ...
        # 7. Проверка объемов
        print("\n7. Проверка объемов:")
        cursor.execute("SELECT COUNT(*) FROM data_5min WHERE volume < 0")
        negative_volume = cursor.fetchone()[0]
        print(f"   Записей с отрицательным объемом: {negative_volume}")
        
        cursor.execute("SELECT SUM(volume), AVG(volume), MAX(volume) FROM data_5min")
        ...
        for col in columns_to_check:
            cursor.execute(f"SELECT COUNT(*) FROM data_5min WHERE {col} IS NULL")
        ...
        print("\n9. Проверка диапазона created_at:")
        cursor.execute("SELECT MIN(created_at), MAX(created_at) FROM data_5min")
        ...
        # 12. Проверка качества 5-минутных данных
        print("\n12. Проверка качества 5-минутных данных:")
        
        # Проверяем, что данные действительно 5-минутные (интервал ~5 минут)
        cursor.execute("""
            SELECT symbol, 
                   COUNT(*) as records,
                   MIN(datetime) as first_time,
                   ...
                   ROUND(COUNT(*) / ((julianday(MAX(datetime)) - julianday(MIN(datetime))) * 24 * 60) * 5, 2) as records_per_5min
            FROM data_5min 
            GROUP BY symbol 
            HAVING COUNT(*) > 20
            ORDER BY records DESC
        ...
        print("   Символ | Записей | Период (мин) | Записей/мин | Записей/5мин | Качество")
        print("   " + "-" * 80)
        for row in minute_quality:
            records_per_5min = row[6] if row[6] else 0
            quality = "EXCELLENT" if 0.8 <= records_per_5min <= 1.2 else "GOOD" if 0.5 <= records_per_5min <= 1.5 else "POOR"
            print(f"   {row[0]:<6} | {row[1]:>7} | {row[4]:>11.1f} | {row[5]:>11.2f} | {row[6]:>12.2f} | {quality}")

        # 13. Проверка временных интервалов (должны быть кратны 5 минутам)
        print("\n13. Проверка временных интервалов:")
        cursor.execute("""
            SELECT symbol, 
                   datetime,
                   LAG(datetime) OVER (PARTITION BY symbol ORDER BY datetime) as prev_datetime,
                   (julianday(datetime) - julianday(LAG(datetime) OVER (PARTITION BY symbol ORDER BY datetime))) * 24 * 60 as interval_minutes
            FROM data_5min 
            WHERE symbol IN (SELECT symbol FROM data_5min GROUP BY symbol ORDER BY COUNT(*) DESC LIMIT 3)
            ORDER BY symbol, datetime
            LIMIT 20
        """)
        
        interval_check = cursor.fetchall()
        print("   Символ | Текущее время | Предыдущее время | Интервал (мин)")
        print("   " + "-" * 70)
        for row in interval_check:
            if row[2]:  # Если есть предыдущее время
                interval = row[3] if row[3] else 0
                print(f"   {row[0]:<6} | {row[1]:<15} | {row[2]:<16} | {interval:>13.1f}")
            else:
                print(f"   {row[0]:<6} | {row[1]:<15} | {'FIRST':<16} | {'N/A':>13}")
        ...
        print("\nТест качества данных data_5min завершен!")
